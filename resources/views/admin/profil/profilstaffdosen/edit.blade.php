...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('Dosen FST') }}</h1>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="./">{{ __('Home') }}</a></li>
            <li class="breadcrumb-item">{{ __('Profil') }}</li>
            <li class="breadcrumb-item">{{ __('Dosen FST') }}</li>
        ...
        <div class="col-lg-12">
            <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">{{ __('Edit Dosen FST') }}</h6>
                ...
                <div class="table-responsive p-3">
                    <form action="{{ route('staff-dosen-fst.update', $dosen->id) }}" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                                ...
                                <div class="col border-right">
                                    <div class="p-3 py-5">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <h4 class="text-right">Dosen FST</h4>
                                        </div>
                                        <div class="row mt-2">
                                            <div class="col-md-12">
                                                ...
                                                <select class="form-control" name="title" @error('title') is-invalid @enderror placeholder="judul">
                                                    ...
                                                    <option @if($dosen->title == "INFORMATIKA") selected @endif value="INFORMATIKA">INFORMATIKA</option>
                                                    <option @if($dosen->title == "SISTEM INFORMASI") selected @endif value="SISTEM INFORMASI">SISTEM INFORMASI</option>
                                                    <option @if($dosen->title == "TEKNIK INDUSTRI") selected @endif value="TEKNIK INDUSTRI">TEKNIK INDUSTRI</option>
                                                </select>
                                                @error('title')
                                                    <small class="text-danger">{{ $message }}</small>
                                                @enderror

...
class KalenderAkademikController extends Controller
{
        ...
        if ($request->ajax()) {
            return DataTables::of($kalenderAkademik)
                ...
                ->addColumn('image-content', function ($row) {
                    $content = '<img src="/Images/kalender-akademik-fakultas/' . $row->image_content . '" alt="kalender-akademik" title="FST" width="100px"/>';
                    return $content;
                })
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<a href="' . route('kalender-akademik-fst.edit', $row->id) . '" data-toggle="tooltip" data-original-title="Edit" class="edit btn btn-primary btn-sm"><span><i class="fas fa-pen-square"></i></span></a>';
                    $btn = $btn . '&nbsp;';
    ...
    public function header(Request $request)
    {
        ...
        return redirect('admin/akademik/kalender-akademik-fst');
    }
    ...
    public function store(Request $request)
    {
        ...
        if($akademik){
            if(isset($data['image_content'])){
                ...
                return redirect()->route('kalender-akademik-fst.index');
        } else {
    ...
    public function update(Request $request, $kalender_akademik)
    {
        ...
        if($request['image_content'] == null) {
            $request['image_content'] = $kalender['image_content'];
        ...
        if ($request->hasfile('image_content')) {
            ...
            $nama_file = time() . str_replace(" ", "", $file->getClientOriginalName());
            $file->move($path, $nama_file);
            if (File::exists('Images/kalender-akademik-fakultas/'.$kalender['image_content'])) {
                File::delete('Images/kalender-akademik-fakultas/'.$kalender['image_content']);
            }
            $data['image_content'] = $nama_file;
        }
    ...
    {
        $fst_kalender = ContentAcademic::Find($kalender_akademik);

        if (File::exists("Images/kalender-akademik-fakultas/".$fst_kalender->image_content)) {
            File::delete("Images/kalender-akademik-fakultas/".$fst_kalender->image_content);
        }

        if (File::exists("files/akademik/".$fst_kalender->content)) {
            File::delete("files/akademik/".$fst_kalender->content);
        }
